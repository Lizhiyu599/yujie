...
function _calGetLunar(year, month, day) {
    ...
    for (lunarMonthIdx = 1; lunarMonthIdx <= 12 && offset > 0; lunarMonthIdx++) {
        ...
        offset -= mDays;
        if (lunarMonthIdx === leapMonth) {
            mDays = _calLeapMonthDays(lunarYear);
            if (offset < mDays) { isLeap = true; break; }
            offset -= mDays;
        }
    }
    lunarMonth = lunarMonthIdx;
    lunarDay = offset + 1;
    
    if (lunarDay === 1) {
        return (isLeap ? '闰' : '') + _lunarMonthNames[lunarMonth - 1] + '月';
    }
    return _lunarDayNames[lunarDay - 1];
}
function _calLunarYearDays(y) {
    var sum = 348;
    for (var i = 0x8000; i > 0x8; i >>= 1) {
        sum += (_lunarInfo[y - 1900] & i) ? 1 : 0;
    }
    return sum + _calLeapMonthDays(y);
}
function _calLeapMonthDays(y) {
    if (_lunarInfo[y - 1900] & 0xf) {
        return (_lunarInfo[y - 1900] & 0x10000) ? 30 : 29;
    }
    return 0;
}
function _calLunarMonthDays(y, m) {
    return (_lunarInfo[y - 1900] & (0x10000 >> m)) ? 30 : 29;
}
function _calSolarMonthDays(y, m) {
...
function openCalendar() {
    ...
    _calLoadEvents();
    var now = new Date();
    _calYear = now.getFullYear(); _calMonth = now.getMonth() + 1; _calSelectedDay = now.getDate();
    _calRenderMonth();
...
function _calRenderMonth() {
    ...
    var lunarStr = _calGetLunar(_calYear, _calMonth, _calSelectedDay);
    ...
    appWindow.innerHTML = '<div class="cal-app">'
        ...
        + '<div class="cal-events-title">' + _calMonth + '月' + _calSelectedDay + '日 <span style="color:#8e8e93;font-size:12px;margin-left:8px;">' + lunarStr + '</span></div>'
        + eventsHTML
        + '</div>'
        + '<div class="cal-period-switch">'
        + '<div class="cal-switch-row"><span>经期开始了吗？</span><input type="checkbox" class="ios-switch-sm" id="calPeriodStartSwitch" ' + (_calPeriodStart ? 'checked' : '') + ' onchange="_calTogglePeriodStart(this.checked)"></div>'
        + '<div class="cal-switch-row"><span>经期结束了吗？</span><input type="checkbox" class="ios-switch-sm" id="calPeriodEndSwitch" ' + (_calPeriodEnd ? 'checked' : '') + ' onchange="_calTogglePeriodEnd(this.checked)"></div>'
        + '</div>'
        + '</div></div>';
}
...
function _calTogglePeriodStart(checked) {
    if (checked) {
        _calPeriodStart = _calYear + '-' + _calMonth + '-' + _calSelectedDay;
        _calPeriodEnd = null;
    } else {
...
function _calTogglePeriodEnd(checked) {
    if (checked) {
        _calPeriodEnd = _calYear + '-' + _calMonth + '-' + _calSelectedDay;
        _calPeriodStart = null;
    } else {
        _calPeriodEnd = null;
    }
...
function _calAddEvent(d) {
    ...
    overlay.className = 'caption-modal-overlay'; overlay.id = 'calEventOverlay';
...
function _calConfirmEvent() {
    ...
    var type = window._calPickedType || 'period'; var d = window._calEventDay || _calSelectedDay;
