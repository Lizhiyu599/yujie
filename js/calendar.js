...
var _calCycleDays = parseInt(localStorage.getItem('cal_cycle_days') || '28');
...
function _calLoadEvents() {
    ...
    _calCycleDays = parseInt(localStorage.getItem('cal_cycle_days') || '28');
}
...
function _calOpenSchedule() {
    _calCloseMenu();
    var appWindow = document.getElementById('calendarAppWindow');
    if (!appWindow) return;
    _calRenderSchedule(appWindow);
}

function _calRenderSchedule(appWindow) {
    var schedule = JSON.parse(localStorage.getItem('cal_schedule') || '{}');
    var days = ['周一','周二','周三','周四','周五','周六','周日'];
    var timeSlots = ['08:00','09:00','10:00','11:00','12:00','13:00','14:00','15:00','16:00','17:00','18:00','19:00'];
    
    var gridHTML = '<div class="cal-schedule-header"><div class="cal-schedule-time-label">时间</div>';
    days.forEach(function(d) { gridHTML += '<div class="cal-schedule-day-label">' + d + '</div>'; });
    gridHTML += '</div>';
    
    timeSlots.forEach(function(t) {
        gridHTML += '<div class="cal-schedule-row"><div class="cal-schedule-time">' + t + '</div>';
        days.forEach(function(d, di) {
            var key = di + '_' + t;
            var course = schedule[key];
            gridHTML += '<div class="cal-schedule-cell' + (course ? ' has-course' : '') + '" onclick="_calEditCourse(' + di + ',\'' + t + '\')">' + (course ? '<div class="cal-course-name">' + course.name + '</div><div class="cal-course-room">' + (course.room || '') + '</div>' : '') + '</div>';
        });
        gridHTML += '</div>';
    });
...
function _calGetDayType(year, month, day) {
    ...
    // 预测下次经期：从本次经期第1天 + 周期天数
    var nextStart = new Date(start);
    nextStart.setDate(nextStart.getDate() + _calCycleDays);
    ...
    // 排卵期：排卵日 = 下次经期 - 14天，排卵期 = 排卵日前5天+排卵日+后4天=10天
    var ovulationDay = new Date(nextStart);
    ovulationDay.setDate(ovulationDay.getDate() - 14);
    var ovStart = new Date(ovulationDay);
    ovStart.setDate(ovStart.getDate() - 5);
    var ovEnd = new Date(ovulationDay);
...
function _calRenderMonth() {
    ...
    for (var d = 1; d <= daysInMonth; d++) {
        var isToday = (_calYear === now.getFullYear() && _calMonth === (now.getMonth()+1) && d === now.getDate());
    ...
    if (!_calPeriodStart && !_calPeriodEnd) {
        ...
        var startDate = new Date(_calPeriodStart);
        var todayDate = new Date();
        var diffDays = Math.floor((todayDate - startDate) / (1000 * 60 * 60 * 24)) + 1;
        if (diffDays > 0) {
            periodSwitchHTML = '<div class="cal-switch-row"><span>经期第' + diffDays + '天</span></div>';
        }
        ...
        var nextStart = new Date(_calPeriodStart);
        nextStart.setDate(nextStart.getDate() + _calCycleDays);
        var diffDays = Math.ceil((nextStart - new Date()) / (1000 * 60 * 60 * 24));
        if (diffDays > 0) {
            periodSwitchHTML = '<div class="cal-switch-row"><span>预计' + diffDays + '天后下次经期</span></div>';
        }
        periodSwitchHTML += '<div class="cal-switch-row"><span>周期</span><span style="display:flex;align-items:center;gap:4px;"><span onclick="_calChangeCycle(-1)" style="cursor:pointer;font-size:18px;color:#8e8e93;">−</span><span style="font-size:14px;font-weight:500;">' + _calCycleDays + '天</span><span onclick="_calChangeCycle(1)" style="cursor:pointer;font-size:18px;color:#8e8e93;">+</span></span></div>';
    }

    appWindow.innerHTML = '<div class="cal-app">'
        + '<div class="cal-nav"><div class="cal-nav-back" onclick="closeCalendar()">‹</div><div class="cal-nav-title">日历</div><div class="cal-nav-menu" onclick="event.stopPropagation();_calOpenMenu()"><span class="cal-menu-dot"></span><span class="cal-menu-dot"></span><span class="cal-menu-dot"></span></div></div>'
        + '<div class="cal-body">'
        ...
        + '<span class="cal-legend-item"><span class="cal-legend-dot" style="background:#ff6b6b;"></span>经期</span>'
        + '<span class="cal-legend-item"><span class="cal-legend-dot" style="background:#ffb3b3;"></span>预测经期</span>'
        + '<span class="cal-legend-item"><span class="cal-legend-dot" style="background:#64b5f6;"></span>排卵期</span>'
        + '<span class="cal-legend-item"><span class="cal-legend-dot" style="background:#a8d8a8;"></span>纪念日</span>'
        + '<span class="cal-legend-item"><span class="cal-legend-dot" style="background:#c0c0c0;"></span>生日</span>'
        + '</div>'
...
function _calChangeCycle(delta) {
    _calCycleDays = Math.max(20, Math.min(45, _calCycleDays + delta));
    localStorage.setItem('cal_cycle_days', _calCycleDays);
    _calRenderMonth();
}

function _calTogglePeriodStart(checked) {
    if (checked) { _calPeriodStart = _calYear + '-' + _calMonth + '-' + _calSelectedDay; _calPeriodEnd = null; }
    else { _calPeriodStart = null; _calPeriodEnd = null; }
...
function _calOpenMenu() {
    ...
    overlay.innerHTML = '<div class="music-menu-panel" onclick="event.stopPropagation();">'
        ...
        + '</div>';
    document.body.appendChild(overlay);
    overlay.onclick = function(e) { if (e.target === overlay) _calCloseMenu(); };
}
function _calCloseMenu() { var o = document.getElementById('calMenuOverlay'); if (o) o.remove(); }
