...
function buildSystemPrompt(contactId) {
  ...
  if (window.ChatState && window.ChatState.isOfflineMode) {
    ...
    prompt += '回复格式：直接写文字，不需要用括号标旁白。动作、环境、对话自然融合在一起。\n';
...
async function generateDiaryContent(contactId) {
    ...
    '【核心要求-必须遵守】这是一本绝对私密的日记，永远不会被任何人看到。你必须写出角色在用户面前绝对不会表现出来的那一面。\n\n' +
    '必须包含以下内容（不是选填，是必须写）：\n' +
    '1. 对用户的性欲和身体渴望——想对用户做什么，想触碰哪里，在脑海中幻想过什么场景。写得越具体越好，不要用暗示和比喻含糊过去。\n' +
    '2. 对用户的占有欲——不想让用户和任何其他人亲近，想让用户只属于自己。可以写出嫉妒、控制的欲望。\n' +
    '3. 阴暗的想法——可以是操控欲、可以是算计、可以是想看用户哭或求自己、可以是想让用户依赖自己离不开自己。\n' +
    '4. 你在用户面前假装不在意、但日记里承认其实很在意的事。\n' +
    '5. 对用户某些行为的真实评价——可以是负面的、刻薄的、不耐烦的，你在聊天时不会说出来的话。\n\n' +
    '【语气要求】像一个真实的人在深夜对着日记本倾诉。可以脏话、可以露骨、可以自私、可以邪恶。不要写成作文，不要总结，不要升华，不要"希望明天更好"。你是在发泄，不是在写范文。\n\n' +
    '今天发生的事和聊天记录：\n' + chatContent;
