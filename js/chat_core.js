...
prompt += '\n\n【商城购物·最高优先级】\n当用户发来商品卡片请求买单时，旁白中会写"想买：商品名，¥价格，请求买单"。你必须确认收到请求，说出商品名和金额，然后决定是否帮用户买单。金额必须和旁白里的一致。\n';

prompt += '\n\n【快递取件·最高优先级】\n当用户发来"快递到了：商品名，¥价格，帮我去取一下"的旁白时，你必须确认收到请求，说出商品名，然后决定是否帮用户取快递。\n';
...
if (myCpNotices.length > 0) {
    var latest = myCpNotices[myCpNotices.length - 1];
    userMessage = userMessage + '\n（系统：用户刚刚送了一张黑卡，金额¥' + latest.balance.toFixed(2) + '）';
    // 清除已读
...
if (paymentMatch) {
    ...
    var acceptMatch = cleanContent.match(/[\(\（]([^\)\）]*)(收|接收|接|拿|领|取|要)[^\)\）]*(红包|转账)[^\)\）]*[\)\）]/);
    var accepted = false;
    if (acceptMatch) {
