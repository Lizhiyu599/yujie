...
function renderQianban() {
    ...
    // 中心节点
    var centerClick = centerType === 'npc'
        ? 'openQianbanDetail(\'' + qbCurrentCenter + '\')'
        : 'switchQianbanCenter(\'' + qbCurrentCenter + '\')';
...
function openAddRelation() {
    ...
    contacts.forEach(function(c) {
        fromHTML += '<div class="qb-add-option" onclick="selectRelationFrom(\'' + c.id + '\', this)">' + c.name + '</div>';
    });
    fromHTML += '</div></div>';
    ...
    npcs.forEach(function(n) {
        fromHTML += '<div class="qb-add-option" onclick="selectRelationFrom(\'' + n.id + '\', this)">' + n.name + '</div>';
    });
    fromHTML += '</div></div>';
    ...
    contacts.forEach(function(c) {
        toHTML += '<div class="qb-add-option" onclick="selectRelationTo(\'' + c.id + '\', this)">' + c.name + '</div>';
    });
    toHTML += '</div></div>';
    ...
    npcs.forEach(function(n) {
        toHTML += '<div class="qb-add-option" onclick="selectRelationTo(\'' + n.id + '\', this)">' + n.name + '</div>';
    });
    toHTML += '</div></div>';
...
function selectRelationFrom(id, el) {
    if (qbSelectedFrom && qbSelectedFrom.id === id) {
        qbSelectedFrom = null;
...
function refreshToOptions(excludeId) {
    ...
    if (roleContainer) {
        ...
        roleContainer.innerHTML = roleHTML || '<div style="color:#8e8e93;font-size:12px;padding:4px;">暂无</div>';
    }
    if (npcContainer) {
        var npcHTML = '';
        npcs.forEach(function(n) {
        ...
        npcContainer.innerHTML = npcHTML || '<div style="color:#8e8e93;font-size:12px;padding:4px;">暂无</div>';
    }
    qbSelectedTo = null;
}
...
function renderTagSection() {
    ...
    if (hasUser) {
        ...
        RELATION_TAGS.forEach(function(t) {
            typeOptions1 += '<div class="qb-add-option" onclick="selectRelationTag(\'' + t + '\', \'fromTo\', this)">' + t + '</div>';
        });
        typeOptions1 += '<div class="qb-add-option" onclick="showCustomTagInput(\'fromTo\', this)">自定义</div>';
        ...
        section.innerHTML = ''
            ...
            + '<div class="qb-add-options" id="qbFromToTypeOptions">' + typeOptions1 + '</div>'
            + '<div id="qbCustomFromToInput" style="display:none;margin:8px 0;">'
            + '<input type="text" class="ios-input" id="qbCustomFromToField" placeholder="自定义">'
            + '</div>'
            ...
            + '<input type="text" class="ios-input" id="qbCustomToFromField" placeholder="自定义">'
            + '</div>';
    } else {
        section.innerHTML = ''
            + '<div style="font-size:13px;color:#8e8e93;margin:12px 0 8px;">A对B的感情</div>'
            + '<div style="margin:0 0 8px;"><input type="text" class="ios-input" id="qbCustomAtoB" placeholder="A对B的感情"></div>'
            + '<div style="font-size:13px;color:#8e8e93;margin:8px 0;">B对A的感情</div>'
            + '<div style="margin:0 0 8px;"><input type="text" class="ios-input" id="qbCustomBtoA" placeholder="B对A的感情"></div>';
    }
    qbSelectedFromToType = null;
    qbSelectedToFromType = null;
}
...
function confirmAddRelation() {
    ...
    if (hasUser) {
        fromTo = qbSelectedFromToType || '';
        if (fromTo === '__custom__') {
            fromTo = (document.getElementById('qbCustomFromToField') || {}).value || '';
        }
        toFrom = qbSelectedToFromType || '';
        if (toFrom === '__custom__') {
            toFrom = (document.getElementById('qbCustomToFromField') || {}).value || '';
        }
    } else {
...
function updateQbNpcGenderRadio() {
    document.querySelectorAll('input[name="qbNpcGender"]').forEach(function(r) {
        if (r.checked) {
...
function updateQbEditNpcGenderRadio() {
    document.querySelectorAll('input[name="qbEditNpcGender"]').forEach(function(r) {
        if (r.checked) {
            r.style.borderColor = '#1d1d1f';
            r.style.background = '#1d1d1f';
            r.style.boxShadow = 'inset 0 0 0 4px #fff';
...
function openQianbanDetail(nodeId) {
    ...
    var nodeAvatar = getNodeAvatar(nodeId);
    ...
    var relatedHTML = '';
    if (related.length === 0) {
        relatedHTML = '<div style="text-align:center;color:#8e8e93;padding:20px;">暂无关系</div>';
    } else {
        related.forEach(function(r) {
            var otherId = r.from === nodeId ? r.to : r.from;
            var otherName = getNodeName(otherId);
            var dir = r.from === nodeId ? '→' : '←';
            var tag = r.from === nodeId ? (r.fromToType || '') : (r.toFromType || '');
            relatedHTML += '<div class="qb-detail-stat"><span>' + dir + ' ' + otherName + '</span><span class="stat-value">' + tag + '</span></div>';
        });
    }
    ...
    overlay.innerHTML = ''
        ...
        + '<div class="qb-detail-meta">' + (nodeType === 'npc' ? 'NPC' : nodeType === 'user' ? '用户' : '角色') + '</div>'
