...
function getNodeName(nodeId) {
    if (nodeId === 'user') { var u = getActiveUserInfo(); return u.name || '我'; }
    var contacts = window.ChatConfig && window.ChatConfig.contacts ? window.ChatConfig.contacts : [];
    var c = contacts.find(function(x) { return x.id === nodeId; }); if (c) return c.name;
    var npcs = getQianbanData().npcs || []; var n = npcs.find(function(x) { return x.id === nodeId; }); if (n) return n.name;
    return '未知';
}
...
function getNodeAvatar(nodeId) {
    ...
    var contacts = window.ChatConfig && window.ChatConfig.contacts ? window.ChatConfig.contacts : [];
    var c = contacts.find(function(x) { return x.id === nodeId; }); if (c) return c.avatarData || '';
    var npcs = getQianbanData().npcs || []; var n = npcs.find(function(x) { return x.id === nodeId; }); if (n) return n.avatar || '';
    return '';
}
...
function getActiveUserInfo() {
    var masks = getMasks ? getMasks() : []; var activeId = getActiveMaskId();
    var m = null; for (var i = 0; i < masks.length; i++) { if (masks[i].id === activeId) { m = masks[i]; break; } }
...
function calcNodePositions(nodeCount, centerX, centerY, rx, ry) {
    ...
    for (var i = 0; i < nodeCount; i++) {
        ...
        positions.push({ x: centerX + rx * Math.cos(angle), y: centerY + ry * Math.sin(angle) });
    }
    return positions;
}
...
function closeQianban() { var w = document.getElementById('qianbanAppWindow'); if (w) w.style.display = 'none'; }
...
function renderQianban() {
    ...
    var others = [];
    contacts.forEach(function(c) { if (c.id !== qbCurrentCenter && relatedIds[c.id]) others.push({ id: c.id, name: c.name, type: 'role', avatar: c.avatarData || '', avatarText: c.avatar }); });
    npcs.forEach(function(n) { if (n.id !== qbCurrentCenter && relatedIds[n.id]) others.push({ id: n.id, name: n.name, type: 'npc', avatar: n.avatar || '', avatarText: n.name.charAt(0) }); });

    var centerName = getNodeName(qbCurrentCenter), centerAvatar = getNodeAvatar(qbCurrentCenter), centerType = getNodeType(qbCurrentCenter);
    ...
    relations.forEach(function(r) {
        ...
        if (r.from === qbCurrentCenter) { fx = cx; fy = cy; var tp = positions[tIdx]; tx = tp.x; ty = tp.y; }
        else if (r.to === qbCurrentCenter) { tx = cx; ty = cy; var fp = positions[fIdx]; fx = fp.x; fy = fp.y; }
        else { var fp = positions[fIdx], tp = positions[tIdx]; fx = fp.x; fy = fp.y; tx = tp.x; ty = tp.y; }
    ...
    var nodesHTML = '';
    var centerClick = centerType === 'npc' ? 'openQianbanDetail(\'' + qbCurrentCenter + '\')' : 'switchQianbanCenter(\'' + qbCurrentCenter + '\')';
    ...
    others.forEach(function(node, i) {
        var pos = positions[i];
        var avt = node.avatar ? '<div class="qb-node-avatar" style="background-image:url(' + node.avatar + ');background-size:cover;background-position:center;">&nbsp;</div>' : '<div class="qb-node-avatar">' + (node.name ? node.name.charAt(0) : '?') + '</div>';
        var click = node.type === 'npc' ? 'openQianbanDetail(\'' + node.id + '\')' : 'switchQianbanCenter(\'' + node.id + '\')';
    ...
    appWindow.innerHTML = '<div class="qianban-app"><div class="qb-top-bar"><div class="qb-back-btn" onclick="closeQianban()">‹</div><div class="qb-title">牵 绊</div><div class="qb-settings-btn" onclick="openQianbanSettings()">○</div></div><div class="qb-graph-area" style="position:relative;"><svg style="position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;">' + svgElements + '</svg>' + nodesHTML + '</div><div class="qb-legend"><div class="qb-legend-item"><span class="qb-legend-dot user"></span>用户</div><div class="qb-legend-item"><span class="qb-legend-dot role"></span>角色</div><div class="qb-legend-item"><span class="qb-legend-dot npc"></span>NPC</div></div><div class="qb-bottom-bar"><button class="qb-btn qb-btn-white" onclick="openAddRelation()">+ 添加关系</button><button class="qb-btn qb-btn-black" onclick="openAddNPC()">+ 添加NPC</button></div></div>';
}

function switchQianbanCenter(nodeId) { qbCurrentCenter = nodeId; renderQianban(); }

// ========== 设置面板 ==========
function openQianbanSettings() {
    var masks = getMasks ? getMasks() : []; var activeId = getActiveMaskId();
    var data = getQianbanData(); var npcs = data.npcs || []; var relations = data.relations || [];
    var maskHTML = '<div class="qb-mask-grid">';
    masks.forEach(function(m) {
        var isActive = m.id === activeId;
        var avt = m.avatar ? '<div style="width:36px;height:36px;border-radius:50%;background-image:url(' + m.avatar + ');background-size:cover;background-position:center;flex-shrink:0;"></div>' : '<div style="width:36px;height:36px;border-radius:50%;background:#e5e5ea;display:flex;align-items:center;justify-content:center;font-size:16px;color:#8e8e93;flex-shrink:0;">' + (m.name ? m.name.charAt(0) : '?') + '</div>';
        maskHTML += '<div class="qb-mask-item' + (isActive ? ' active' : '') + '" onclick="selectQianbanMask(\'' + m.id + '\')">' + avt + '<span style="font-size:13px;">' + m.name + '</span></div>';
    });
    maskHTML += '</div>';
    var nHTML = ''; if (npcs.length === 0) nHTML = '<div style="text-align:center;color:#8e8e93;padding:16px;">暂无NPC</div>'; else npcs.forEach(function(n) { nHTML += '<div class="qb-edit-row" style="justify-content:space-between;"><span>' + n.name + '</span><span style="color:#ff3b30;font-size:13px;cursor:pointer;" onclick="deleteNPC(\'' + n.id + '\')">删除</span></div>'; });
    var rHTML = ''; if (relations.length === 0) rHTML = '<div style="text-align:center;color:#8e8e93;padding:12px;">暂无关系</div>'; else relations.forEach(function(r) { rHTML += '<div class="qb-edit-row" style="justify-content:space-between;"><span style="font-size:13px;">' + getNodeName(r.from) + ' ↔ ' + getNodeName(r.to) + '</span><span style="color:#ff3b30;font-size:13px;cursor:pointer;" onclick="deleteRelation(\'' + r.id + '\')">删除</span></div>'; });
    var overlay = document.createElement('div'); overlay.className = 'qb-edit-overlay'; overlay.id = 'qbSettingsOverlay';
    overlay.innerHTML = '<div class="qb-edit-panel" onclick="event.stopPropagation()"><div class="qb-edit-handle"></div><div class="qb-edit-title">牵绊设置</div><div style="font-size:13px;color:#8e8e93;margin-bottom:8px;">切换面具关系网</div>' + maskHTML + '<div style="font-size:13px;color:#8e8e93;margin:16px 0 8px;">管理关系</div>' + rHTML + '<div style="font-size:13px;color:#8e8e93;margin:16px 0 8px;">管理NPC</div>' + nHTML + '<div class="qb-edit-buttons"><div class="qb-edit-btn-cancel" onclick="closeQianbanSettings()">关闭</div></div></div>';
    document.body.appendChild(overlay); overlay.onclick = function(e) { if (e.target === overlay) closeQianbanSettings(); };
}

function deleteRelation(relId) { var d = getQianbanData(); d.relations = d.relations.filter(function(r) { return r.id !== relId; }); saveQianbanData(d); closeQianbanSettings(); showToast('关系已删除'); renderQianban(); }
...
function deleteNPC(id) { var d = getQianbanData(); d.npcs = d.npcs.filter(function(n) { return n.id !== id; }); d.relations = d.relations.filter(function(r) { return r.from !== id && r.to !== id; }); saveQianbanData(d); closeQianbanSettings(); showToast('NPC已删除'); renderQianban(); }

// ========== 添加关系面板 ==========
function openAddRelation() {
    var data = getQianbanData(); var contacts = window.ChatConfig && window.ChatConfig.contacts ? window.ChatConfig.contacts : []; var npcs = data.npcs || []; var userInfo = getActiveUserInfo();
    var fHTML = '<div class="qb-add-section"><div class="qb-add-section-title">我的面具</div><div class="qb-add-options" id="qbFromUserOptions"><div class="qb-add-option selected" onclick="selectRelationFrom(\'user\',this)">' + userInfo.name + '</div></div></div>';
    fHTML += '<div class="qb-add-section-divider"></div><div class="qb-add-section"><div class="qb-add-section-title">角色</div><div class="qb-add-options" id="qbFromRoleOptions">'; contacts.forEach(function(c) { fHTML += '<div class="qb-add-option" onclick="selectRelationFrom(\'' + c.id + '\',this)">' + c.name + '</div>'; }); fHTML += '</div></div>';
    fHTML += '<div class="qb-add-section-divider"></div><div class="qb-add-section"><div class="qb-add-section-title">NPC</div><div class="qb-add-options" id="qbFromNpcOptions">'; npcs.forEach(function(n) { fHTML += '<div class="qb-add-option" onclick="selectRelationFrom(\'' + n.id + '\',this)">' + n.name + '</div>'; }); fHTML += '</div></div>';
    var tHTML = '<div class="qb-add-section"><div class="qb-add-section-title">角色</div><div class="qb-add-options" id="qbToRoleOptions">'; contacts.forEach(function(c) { tHTML += '<div class="qb-add-option" onclick="selectRelationTo(\'' + c.id + '\',this)">' + c.name + '</div>'; }); tHTML += '</div></div>';
    tHTML += '<div class="qb-add-section-divider"></div><div class="qb-add-section"><div class="qb-add-section-title">NPC</div><div class="qb-add-options" id="qbToNpcOptions">'; npcs.forEach(function(n) { tHTML += '<div class="qb-add-option" onclick="selectRelationTo(\'' + n.id + '\',this)">' + n.name + '</div>'; }); tHTML += '</div></div>';
    var overlay = document.createElement('div'); overlay.className = 'qb-edit-overlay'; overlay.id = 'qbAddRelationOverlay';
    overlay.innerHTML = '<div class="qb-edit-panel" onclick="event.stopPropagation()"><div class="qb-edit-handle"></div><div class="qb-edit-title">添加关系</div><div style="font-size:13px;color:#8e8e93;margin-bottom:8px;">关系起点（点击取消选中）</div>' + fHTML + '<div style="font-size:13px;color:#8e8e93;margin:12px 0 8px;">关系终点</div>' + tHTML + '<div id="qbTagSection"></div><div class="qb-edit-buttons"><div class="qb-edit-btn-cancel" onclick="closeAddRelation()">取消</div><div class="qb-edit-btn-confirm" onclick="confirmAddRelation()">确认</div></div></div>';
    document.body.appendChild(overlay); overlay.onclick = function(e) { if (e.target === overlay) closeAddRelation(); };
    qbSelectedFrom = { id: 'user' }; qbSelectedTo = null; qbSelectedFromToType = null; qbSelectedToFromType = null; renderTagSection();
}

var qbSelectedFrom = { id: 'user' }, qbSelectedTo = null, qbSelectedFromToType = null, qbSelectedToFromType = null;

function selectRelationFrom(id, el) { if (qbSelectedFrom && qbSelectedFrom.id === id) { qbSelectedFrom = null; el.classList.remove('selected'); } else { qbSelectedFrom = { id: id }; document.querySelectorAll('#qbFromUserOptions .qb-add-option,#qbFromRoleOptions .qb-add-option,#qbFromNpcOptions .qb-add-option').forEach(function(o) { o.classList.remove('selected'); }); el.classList.add('selected'); } refreshToOptions(qbSelectedFrom ? qbSelectedFrom.id : null); renderTagSection(); }
function refreshToOptions(excludeId) { var contacts = window.ChatConfig && window.ChatConfig.contacts ? window.ChatConfig.contacts : []; var npcs = getQianbanData().npcs || []; var rc = document.getElementById('qbToRoleOptions'), nc = document.getElementById('qbToNpcOptions'); if (rc) { var rh = ''; contacts.forEach(function(c) { if (c.id !== excludeId) rh += '<div class="qb-add-option" onclick="selectRelationTo(\'' + c.id + '\',this)">' + c.name + '</div>'; }); rc.innerHTML = rh || '<div style="color:#8e8e93;font-size:12px;padding:4px;">暂无</div>'; } if (nc) { var nh = ''; npcs.forEach(function(n) { if (n.id !== excludeId) nh += '<div class="qb-add-option" onclick="selectRelationTo(\'' + n.id + '\',this)">' + n.name + '</div>'; }); nc.innerHTML = nh || '<div style="color:#8e8e93;font-size:12px;padding:4px;">暂无</div>'; } qbSelectedTo = null; }
function selectRelationTo(id, el) { qbSelectedTo = { id: id }; document.querySelectorAll('#qbToRoleOptions .qb-add-option,#qbToNpcOptions .qb-add-option').forEach(function(o) { o.classList.remove('selected'); }); el.classList.add('selected'); }
...
function renderTagSection() {
    ...
    if (hasUser) {
        var t1 = ''; RELATION_TAGS.forEach(function(t) { t1 += '<div class="qb-add-option" onclick="selectRelationTag(\'' + t + '\',\'fromTo\',this)">' + t + '</div>'; }); t1 += '<div class="qb-add-option" onclick="showCustomTagInput(\'fromTo\',this)">自定义</div>';
        var t2 = ''; RELATION_TAGS.forEach(function(t) { t2 += '<div class="qb-add-option" onclick="selectRelationTag(\'' + t + '\',\'toFrom\',this)">' + t + '</div>'; }); t2 += '<div class="qb-add-option" onclick="showCustomTagInput(\'toFrom\',this)">自定义</div>';
...
function selectRelationTag(type, target, el) { if (target === 'fromTo') { qbSelectedFromToType = type; document.querySelectorAll('#qbFromToTypeOptions .qb-add-option').forEach(function(o) { o.classList.remove('selected'); }); document.getElementById('qbCustomFromToInput').style.display = 'none'; } else { qbSelectedToFromType = type; document.querySelectorAll('#qbToFromTypeOptions .qb-add-option').forEach(function(o) { o.classList.remove('selected'); }); document.getElementById('qbCustomToFromInput').style.display = 'none'; } el.classList.add('selected'); }
function showCustomTagInput(target, el) { if (target === 'fromTo') { qbSelectedFromToType = '__custom__'; document.getElementById('qbCustomFromToInput').style.display = 'block'; } else { qbSelectedToFromType = '__custom__'; document.getElementById('qbCustomToFromInput').style.display = 'block'; } el.classList.add('selected'); }
...
function confirmAddRelation() {
    ...
    var hasUser = qbSelectedFrom.id === 'user'; var fromTo = '', toFrom = '';
    if (hasUser) { fromTo = qbSelectedFromToType || ''; if (fromTo === '__custom__') fromTo = (document.getElementById('qbCustomFromToField') || {}).value || ''; toFrom = qbSelectedToFromType || ''; if (toFrom === '__custom__') toFrom = (document.getElementById('qbCustomToFromField') || {}).value || ''; }
    else { fromTo = (document.getElementById('qbCustomAtoB') || {}).value || ''; toFrom = (document.getElementById('qbCustomBtoA') || {}).value || ''; }
...
function openQianbanDetail(nodeId) {
    var data = getQianbanData(); var contacts = window.ChatConfig && window.ChatConfig.contacts ? window.ChatConfig.contacts : []; var npcs = data.npcs || []; var relations = data.relations || [];
