...
function _crGenerate() {
    ...
    // 人称示例
    var personDemo = '';
    if (_crSelectedPerson === '我') personDemo = '示例开头：我推开门的瞬间，愣住了。';
    else if (_crSelectedPerson === '你') personDemo = '示例开头：你推开门的瞬间，愣住了。';
    ...
    + '8. 最多写3个场景。每个场景要展开：环境、动作、对话、情绪。重头戏（亲密场景、冲突高潮）至少占1000字，禁止跳跃。\n'
    + '9. 亲密场景按步骤展开：触碰、身体反应、对话、情绪变化。每个步骤都要写细节。禁止用"事后""结束后""第二天早上"跳过。\n'
    + '10. 如果用户指定的结局走向涉及亲密关系、身体接触、情感升温，你必须详细描写亲密场景。使用具体器官词汇（嘴唇、手指、乳房、穴、肉棒等），描写生理反应（呼吸变重、身体发热、颤抖、湿润等），描写触碰细节。禁止一笔带过或事后跳过。让读者脸红心跳。\n'
